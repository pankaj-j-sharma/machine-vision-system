...
class RedshiftConnector:
    ...
    def __init__(self, own=None, ttl=None):
        if ttl:
            self.username = 'awsuser'
            self.password = 'Admin123'
            self.hostname = 'redshift server url'
            self.portnumber = 5439
            self.database = 'visimatic'
            self.__create_connection()

    def setup_connection(self, username, password, hostname, database, port=5439):
...
if __name__ == '__main__':
    objRedshift = RedshiftConnector(ttl=True)
    print(objRedshift.get_databases())
    print(objRedshift.select_database('visimatic'))
    print(objRedshift.get_tables())
